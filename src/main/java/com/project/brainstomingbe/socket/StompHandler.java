package com.project.brainstomingbe.socket;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        //만약 COMMAND가 SUBSCRIBE 즉 메세지를 주고 받기전 구독하는 것이라면
            ...
            chatMessageService.sendChatMessage(ChatMessageRequestDto.builder().type(ChatMessage.MessageType.ENTER).roomId(roomId).sender(name).build());
        ...
        //채팅방을 나가는경우
            ...
            if(accessor.getFirstNativeHeader("token") != null) {
                String name = jwtTokenProvider.getAuthenticationUsername(token);
                chatMessageService.sendChatMessage(ChatMessageRequestDto.builder().type(ChatMessage.MessageType.QUIT).roomId(roomId).sender(name).build());
            }
